...
judge uses the OpenAI API so the evaluation runs with the same provider used
for the base and fine-tuned model calls.
...
class JudgeClient:
    """Scores response_speech quality with an OpenAI judge model."""

    def __init__(self) -> None:
        """Create the judge client from the configured OpenAI key.

        Records ``self.backend`` and ``self.model`` for the report.
        """
        ...
        if not key:
            raise RuntimeError("No OPENAI_API_KEY for judge.")
        self.backend = "openai"
